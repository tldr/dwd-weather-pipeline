...
# -------------------- Config --------------------
DWD_BASE_URL = "https://opendata.dwd.de/climate_environment/CDC/observations_germany/climate/daily/kl/recent/"
...
def list_zip_files(station_ids: Optional[List[int]] = None) -> list[str]:
    ...
    if station_ids:
        filtered_links = [z for z in zip_links if any(f"_{station_id:05d}_" in z for station_id in station_ids)]
...
def get_station_metadata() -> pd.DataFrame:
    ...
    df["STATIONS_ID"] = pd.to_numeric(df["STATIONS_ID"].astype(str).str.strip(), errors='coerce').astype(int)
...
def enrich_with_station_metadata(df_weather: pd.DataFrame, df_meta: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"After conversion - Weather data STATIONS_ID type: {df_weather['STATIONS_ID'].dtype}")
